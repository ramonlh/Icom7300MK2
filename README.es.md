...
El repositorio incorpora además un subsistema independiente para el **Quansheng UV-K5**. Su protocolo, transporte serie, modelo de estado y autorización de transmisión permanecen separados del controlador CI-V del Icom.
...
El directorio [`quansheng/`](quansheng/) contiene el desarrollo nativo Linux para el **Quansheng UV-K5**. Procede del proyecto independiente QuanshengDock-Linux y conserva su propio código, protocolo, documentación y pruebas.

La integración mantiene separados:

- el protocolo Icom CI-V;
- el protocolo QuanshengDock/UV-K5;
- el transporte serie;
- el transporte LAN;
- el modelo observable de estado;
- la autorización TX/PTT.

No se reutiliza `RadioController` del Icom como controlador del Quansheng y la integración no modifica el funcionamiento estable del IC-7300MK2.

### Arquitectura actual

El IC-7300MK2 está conectado localmente al HP principal. El Quansheng UV-K5 permanece conectado mediante USB/serie al HP Pavilion dv6, donde el adaptador PL2303 funciona de forma estable.

```text
HP principal
├── IC-7300MK2 local mediante CI-V
└── cliente Quansheng
       │
       └── LAN ──► qdock-server en HP Pavilion dv6
                         │
                         └── USB/serie ──► Quansheng UV-K5
```

El acceso físico al puerto `/dev/ttyUSB0` reside en el Pavilion. La aplicación principal recibe el estado mediante un cliente TCP/JSON independiente y no presupone que ese dispositivo serie exista en el HP principal.

### Funciones implementadas

- Parser incremental del protocolo Quansheng.
- Soporte de tramas `AB CD` y eventos de interfaz `B5`.
- Transporte serie Qt `QSerialPort` y servidor LAN `qdock-server`.
- Cliente LAN integrado en la interfaz principal.
- Reconstrucción pasiva del estado de pantalla.
- Observación de VFO A/B, selector activo, frecuencia, modo, memoria, nombre y potencia mostrada.
- Estado RX, batería, paso, tono, DTMF e indicadores observables.
- Consulta experimental RSSI.
- Lectura experimental de registros BK4819.
- Tabla reservada para las 128 direcciones BK4819 `0x00–0x7F`.
- Interpretación de registros de bloques, AGC, AFC, audio, RF, filtros, squelch, PA, CTCSS/CDCSS, tonos, escáner y DTMF.
- Consulta lenta de hasta 50 registros cada 30 segundos.
- Consulta de frecuencia interna mediante los registros `0x38/0x39` cada dos segundos.
- Lectura EEPROM completa bajo autorización explícita del servidor.
- Interpretación de 200 canales, incluidos canales vacíos.
- Vista de ajustes, opciones de usuario y calibraciones.
- Volcado hexadecimal completo de 8192 bytes.
- Valores de usuario EEPROM desde `0x0E70` hasta `0x0F47`.
- Presentación destacada del canal de llamada `0x0E70`, squelch `0x0E71`, visualización de canal `0x0E79` y cross-band `0x0E7A`.
- Copia al portapapeles de la tabla de registros y del volcado EEPROM.
- Captura cruda opcional y replay de sesiones para pruebas reproducibles.

Los datos procedentes de pantalla que todavía no están plenamente validados se identifican como observaciones candidatas. Los valores de calibración sin unidad confirmada se conservan en bruto y los secretos no se muestran en claro.

### Lectura EEPROM

La lectura EEPROM es experimental y debe habilitarse expresamente al iniciar el servidor con:

```text
--allow-eeprom-query
```

El cliente solicita 8192 bytes en 64 bloques de 128 bytes. La sesión utiliza exclusivamente `Hello 0x0514` y `ReadEeprom 0x051B`; no existe una ruta de escritura EEPROM en la aplicación.

El comando `Hello` puede apagar temporalmente la iluminación de la pantalla del UV-K5. La lectura debe realizarse con la radio bajo observación y detenerse si aparece un comportamiento anómalo.

### Límites de seguridad

Actualmente permanecen bloqueados o sin implementar:

- escritura EEPROM;
- escritura de registros BK4819;
- escritura GPIO;
- cambio de frecuencia o modo del Quansheng;
- teclas remotas;
- TX/PTT;
- flasheo de firmware;
- ejecución de `k5prog -F`;
- modificación de servicios del Pavilion desde la aplicación.

Los campos EEPROM identificados como valores de usuario se muestran actualmente en modo de solo lectura. Su presencia en la interfaz prepara controles futuros, pero no autoriza ni implementa todavía su modificación.

### Compilar y probar el subsistema
...
En un sistema sin Qt SerialPort puede compilarse el núcleo y el replay con funcionalidad limitada:
...
La configuración `QDOCK_SERIAL=OFF` no permite utilizar el puerto serie real.

### Iniciar el servidor del Pavilion

El lanzador de telemetría inicia el servidor con RSSI, registros y lectura EEPROM bajo demanda:
...
- [README del subsistema](quansheng/README.md).
- [Contexto y estado confirmado](quansheng/PROJECT_CONTEXT.md).
- [Protocolo serie](quansheng/docs/PROTOCOL.md).
- [Protocolo LAN](quansheng/docs/LAN_PROTOCOL.md).
- [Reglas específicas de desarrollo](quansheng/AGENTS.md).
